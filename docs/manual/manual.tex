...
A janela principal mostra a placa inteira simultaneamente: as quatro
ECUs em \emph{grid} 2$\times$2 à esquerda, o painel MCU à direita,
mais o injetor e o monitor CAN na coluna direita inferior.  A
figura abaixo é um \emph{snapshot} típico --- placa em execução,
LCD com saída do firmware, valores reais de \emph{Free RAM} nas
barras laterais e tráfego CAN no monitor.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{board_active.png}
  \caption{Sessão em execução. ECU1 mostra ``SPEED 72 km/h'' no LCD,
           ECU2 está em \emph{error-passive} (CAN amarelo, TEC=142),
           potenciômetros das quatro ECUs em ângulos distintos e o
           monitor CAN à direita lista 10 quadros recentes.}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{ecu1_panel.png}
  \caption{Painel da ECU1 em detalhe --- barra lateral à esquerda,
           Nano + LCD no centro, botões DIN à direita, e LEDs +
           potenciômetros + buzzer no rodapé.}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{mcu_panel.png}
  \caption{Painel do MCU. Os dois seletores \texttt{Mode 1}/\texttt{Mode 2}
           geram a máquina de estados de quatro modos
           (\emph{Electric}/\emph{Auto}/\emph{Manual}/\emph{Diag})
           --- veja o exemplo \texttt{mcu\_mode\_select}.}
\end{figure}

\subsection{LCD I\textsuperscript{2}C 16$\times$2}

O LCD desenhado abaixo do Nano em cada ECU/MCU é alimentado
em tempo real pelas instruções TWI do firmware: cada
\texttt{lcd.print()} no sketch atualiza a representação na UI
sem polling adicional.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{ecu1_lcd.png}
  \caption{Close-up do LCD com saída do firmware
           \texttt{dashboard\_full} rodando na ECU1.}
\end{figure}

\subsection{Monitor + injeção CAN}

A coluna direita inferior tem dois \emph{cards} dedicados ao
barramento CAN1: um \emph{form} de injeção (envia um \emph{frame}
arbitrário a partir do PC, identificando-se como \texttt{src=host})
e um monitor com \emph{scroll} mostrando todos os \emph{frames}
recentes (limite de 1000 entradas).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{can_inject.png}
  \caption{Painel de injeção: ID (decimal ou \texttt{0x...}),
           \emph{flags} \emph{ext}/\emph{rtr}, DLC com $\pm$, e o
           campo de \emph{data} em hex (DLC$\times$2 caracteres).
           \emph{Send} dispara um comando \texttt{can\_inject} no
           engine.}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{can_monitor.png}
  \caption{Monitor CAN com tráfego ao vivo. Cada linha é um
           \emph{frame} \texttt{can\_tx} emitido por uma das ECUs;
           o engine entrega os \emph{frames} a todos os outros
           controladores em ordem de \emph{attach}, e o monitor
           registra a sequência observada.}
\end{figure}
...
% =================================================================
% 9. Exemplos de firmware
% =================================================================
\section{Exemplos de firmware}

A pasta \texttt{firmware/examples/} traz \emph{sketches} Arduino que
exercitam cada periférico da placa em isolamento. Cada um vem
acompanhado de \texttt{.ino}, \texttt{.hex} pré-compilado (com e sem
bootloader) e uma \emph{tag} no \texttt{README.md} indicando em qual
ECU faz mais sentido rodar.

\subsection{Catálogo}

\begin{itemize}[nosep]
  \item \textbf{blink}: pisca o LED L (D13) a 1\,Hz. Sanidade básica.
  \item \textbf{io\_test}: \emph{smoke test} que toca em DIN1--4,
        AIN0--3, DOUT0/1, L e LDR\_LED simultaneamente, exibindo o
        estado das entradas no LCD.
  \item \textbf{lcd\_hello}: inicializa o display I\textsuperscript{2}C
        16$\times$2 a 0x27 e exibe duas linhas com texto deslizante.
  \item \textbf{buzzer\_tone}: toca um arpejo de C maior na saída
        BUZZ (PD7) usando \texttt{tone()} / \texttt{noTone()}.
  \item \textbf{adc\_to\_pwm}: lê AIN0 e modula DOUT0 (PD3) via PWM
        de forma linear. Plotador mostra os dois sinais paralelos.
  \item \textbf{button\_counter}: contador de eventos com debounce
        nos botões DIN1--4 e exibição no LCD; DIN1=$+1$, DIN2=$-1$,
        DIN3=zera, DIN4=alterna sinal.
  \item \textbf{serial\_echo}: lê linhas da UART e devolve com
        prefixo numérico. Útil para validar o caminho bridge $\to$
        COM virtual em ambos os sentidos.
  \item \textbf{ldr\_loop} (ECU1): malha óptica fechada --- PWM em
        PD6 acende o LDR\_LED e o sketch lê o LDR em A0, gerando
        uma curva de transferência ótica.
  \item \textbf{loop\_feedback} (ECU2): malha elétrica via D5
        $\to$ A7 com controlador proporcional simples ($K_P = 0,2$).
        Alvo alterna entre 30\,\% e 70\,\% a cada 1,5\,s; saída
        impressa em CSV para o Plotador.
  \item \textbf{can\_heartbeat}: quatro variantes (ECU1--ECU4) que
        transmitem um contador na CAN periodicamente. Demonstra
        comunicação inter-ECU.
  \item \textbf{can\_busoff\_demo}: reporta TEC/REC/EFLG da MCP2515
        a 1\,Hz e se recupera automaticamente de \emph{bus-off}.
        Use junto com a injeção de erros do menu CAN da UI ou o
        \emph{scenario player} para ver as transições.
  \item \textbf{mcu\_mode\_select} (MCU): máquina de estados de 4
        estados (\emph{Electric}, \emph{Auto}, \emph{Manual},
        \emph{Diag}) controlada pelos seletores D8/D9 do MCU.
  \item \textbf{kitchen\_sink}: \emph{todos} os periféricos em um
        sketch só (DOUT, DIN, AIN, PWM, BUZZ, LCD, CAN). Bom para
        verificar que tudo está plugado depois de uma reinstalação.
\end{itemize}
...
O script gera tanto \texttt{<nome>.hex} (sem bootloader, para o caminho
\texttt{cmd\_load} da UI) quanto \texttt{<nome>\_with\_bootloader.hex}
(com Optiboot embutido, para uso com \texttt{avrdude} externo). Como o
\emph{engine} do aneb-sim já pré-carrega Optiboot em \texttt{chip\_init},
qualquer das duas variantes funciona em qualquer caminho de gravação.

\subsection{Fluxo recomendado de aprendizado}

Para uma turma nova:

\begin{enumerate}[nosep]
  \item Comece por \textbf{io\_test} --- vai mostrar todos os widgets
        da UI ao mesmo tempo, dando intimidade com a placa.
  \item Cada periférico em isolado: \textbf{buzzer\_tone},
        \textbf{adc\_to\_pwm}, \textbf{button\_counter},
        \textbf{lcd\_hello}, \textbf{serial\_echo}.
  \item Conceitos de controle: \textbf{ldr\_loop} (ECU1) e
        \textbf{loop\_feedback} (ECU2) introduzem leitura analógica
        + atuação proporcional.
  \item Comunicação: \textbf{can\_heartbeat} a quatro nós, depois
        \textbf{can\_busoff\_demo} com injeção de erros para ver
        bus-off acontecer e ser recuperado.
  \item \textbf{kitchen\_sink} fecha o ciclo --- combine seu próprio
        sketch a partir das partes deste.
\end{enumerate}

% =================================================================
% 10. Cenários (JSON-Lines)
% =================================================================
\section{Cenários: \emph{teaching aids} reproduzíveis}

A pasta \texttt{demos/} contém arquivos \texttt{.jsonl} que o engine
sabe replantar via cronograma absoluto. São roteiros para aulas:
em vez de digitar comandos manualmente, o instrutor seleciona um
cenário e os chips passam pelos estados encenados.

\subsection{Formato}

Um objeto JSON por linha. O objeto é um comando normal do engine
(ver \texttt{docs/PROTOCOL.md}) com um campo extra \texttt{at\_ms}
indicando \emph{quando} (em milissegundos desde o início do script)
aplicá-lo:
...
Linhas começando com \texttt{\#} ou \texttt{//} são comentários e são
ignoradas. Linhas em branco também.

\subsection{Catálogo}

\begin{itemize}[nosep]
  \item \textbf{busoff\_recovery.jsonl}: força ECU2 a entrar em
        \emph{bus-off} duas vezes e se recuperar. Use junto com
        \texttt{can\_busoff\_demo.hex} flashado em ECU2 para ver as
        transições no Console.
  \item \textbf{tec\_climb.jsonl}: incrementa TEC de ECU3 em 16
        unidades a cada 500\,ms, atravessando os limiares
        \emph{active} $\to$ \emph{passive} (TEC=128) e
        \emph{passive} $\to$ \emph{bus-off} (TEC=256), depois
        recupera. Curva fica visível no Plotter.
  \item \textbf{multi\_node\_arbitration.jsonl}: injeta três quadros
        CAN com IDs diferentes no mesmo \emph{tick}, ilustrando a
        ordem de entrega do barramento simulado (em ordem de
        \emph{attach}, não de ID).
\end{itemize}

\subsection{Como executar}

\textbf{Pela UI}: clique \textbf{Toolbar} $\to$ \textbf{Scenario \textbf{\textgreater{}}} e escolha
um item do menu. ``Choose .jsonl file...'' abre um diálogo para
selecionar qualquer arquivo do disco.

\textbf{Pela CLI}: passe \texttt{-{}-script} ao binário do engine ---
útil para demonstrações automatizadas e \emph{screencasts}:

\begin{lstlisting}[language=bash]
./build/aneb-sim/aneb-sim.exe \
    --script=demos/busoff_recovery.jsonl --no-mcu
\end{lstlisting}

\textbf{Por JSON-Lines}: envie um comando \texttt{run\_script}
diretamente para a \emph{stdin} do engine:

\begin{lstlisting}
{"v": 1, "c": "run_script", "path": "demos/busoff_recovery.jsonl"}
\end{lstlisting}

\subsection{Criando seus próprios cenários}

Use qualquer editor de texto. Recomendações:

\begin{itemize}[nosep]
  \item \texttt{at\_ms} é \emph{absoluto} a partir do início do
        script. Se duas linhas têm o mesmo \texttt{at\_ms}, ambas
        disparam tão próximo quanto o sistema operacional permite
        (típico em demos de \emph{race} de barramento).
  \item Se o tempo \texttt{at\_ms} já passou ao chegarmos na linha
        (por algum atraso de \emph{scheduling}), a linha dispara
        imediatamente em vez de \emph{retroativamente}.
  \item Cenários podem ser executados em paralelo --- a fila de
        comandos é única, então as linhas se intercalam pela ordem
        cronológica em que ficam prontas.
\end{itemize}

% =================================================================
% 11. Comandos via JSON-Lines (CLI direta)
% =================================================================
